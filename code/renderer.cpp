...
void
Shader::compile_program(char* vertex_filepath,
                        char* fragment_filepath,
                        char* geometry_filepath)
{
    ...
    if(geometry_filepath) {
        fopen_s(&fp, geometry_filepath, "rb");
        fseek(fp, 0, SEEK_END);
        size = ftell(fp);
        fseek(fp, 0, SEEK_SET);
    ...
    if(geometry_filepath) { glAttachShader(this->program_handle, geo); }
...
void
Shader::set_uniform(char* name, float value)
{
...
void
Shader::set_uniform(char* name, bool value)
{
    glUniform1i(glGetUniformLocation(program_handle, name),
                value);
}
